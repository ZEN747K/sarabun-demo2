...
<script>
        ...
        $('#modalForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // === ตั้งค่าเริ่มต้นและสลับลำดับ: บน → ลายเซ็น → ล่าง ===
                    ...
                    // ลำดับใหม่: textBox → imageBox → bottomBox
                    signatureCoordinates = {
                        ...
                        imageBox: {
                            startX: startX - 13,
                            startY: startY + defaultTextHeight + gap, // ใต้ textBox
                        ...
                        bottomBox: {
                            startX: startX,
                            startY: startY + defaultTextHeight + gap + defaultImageHeight + gap, // ใต้รูป
                            endX: startX + defaultTextWidth,
                    ...
                    function redrawSignatureBoxes() {
                        ...
                        // Draw signature image inside bottomBox then text below
                    ...
                    function isOnResizeHandle(mouseX, mouseY, box) {
                        return (mouseX >= box.endX - resizeHandleSize && mouseX <= box.endX &&
                                mouseY >= box.endY - resizeHandleSize && mouseY <= box.endY);
                    }
                    function isInBox(mouseX, mouseY, box) {
                        return (mouseX >= box.startX && mouseX <= box.endX &&
                    ...
                    function onDragEnd() {
                        isDragging = false; activeBox = null;
                    ...
                    function onResizeEnd() {
                        isResizing = false; activeBox = null;
    ...
    if (selectPageTable) {
        selectPageTable.addEventListener('change', function() {
            let selectedPage = parseInt(this.value); ajaxTable(selectedPage);
        });
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire({
                    ...
                    if (!result.isConfirmed) return;
                    $('#txt_label').text('- แทงเรื่อง (' + result.value.text.join(',') + ') -');
                    $('#users_id').val(result.value.id.join(','));
        ...
        $('#reject-book').click(function (e) {
            ...
            Swal.fire({
                ...
                $.ajax({
                    type: "post",
                    url: "/book/reject",
                    data: { id: $('#id').val(), note: result.value },
                    dataType: "json",
                    headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                    success: function (response) {
                        if (response.status) {
    ...
    function printPdf(url){
        try{
            ...
            if(!w) return; var fired=false; var doPrint=function(){ if(fired) return; fired=true; try{ w.focus(); w.print(); }catch(e){} };
